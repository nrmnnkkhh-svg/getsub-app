...
- **Run #3 addendum:** the jobs.json oracle worked (job lifecycle verified end-to-end: `Queued → Error` with the bot-check message). Two remaining gaps diagnosed: (a) notification record line layouts differ per API level, so id+pkg same-line matching missed existing records → assertions now use format-independent **package presence** (during Queued only the ongoing notification can exist; after teardown only the result), plus `notif_dump.txt`/package dumps saved as artifacts for future forensics, and an "Allow" tap if a permission dialog ever appears; (b) with job rows on screen the window never reaches accessibility idle because the 1s poller rebuilt all rows every tick → fixed in the app as v2.5.4 (§31). Run #4 = verdict.
