...
## 30. CI test-harness fix — run #1 post-mortem (Sep 21, 2026)

- **Run #1 outcome:** build + parser suite + KVM emulator boot all passed; the UI driver passed 9/10 checks (install, launch, every home-UI element) and then timed out in the paste-flow wait. The post-timeout screenshot **proved the app itself worked correctly**: job card rendered `ERROR — YouTube: Sign in to confirm you're not a bot` (expected datacenter-IP outcome), field cleared, notifications posted. The failure was entirely in the test harness.
- **Root causes (two, both in `ci/emulator_ui_test.py`):** (1) `uiautomator dump` waits for the accessibility *idle state* — a screen with an indeterminate progress bar/row spinner/toast/heads-up notification never goes idle, so dumps taken during a fetch hang ~13s; (2) a hung dump writes no file, and `dump_ui()` `cat`ted the previous successful capture from `/sdcard`, so the wait loop kept "seeing" the stale empty-home hierarchy forever. Bonus hazard: a timed-out dump leaves its UiAutomation connection registered, which can hang the *next* dump too.
- **Fix:** service-lifecycle waits now poll `dumpsys notification_manager` (ongoing "Fetching subtitles..." + the per-job "Done:/Error:" result notifications) — completely UI-idle-independent; UI dumps happen only on settled screens (7s settle lets heads-ups retract); `dump_ui()` deletes the device-side dump file before each attempt (a stale capture can never lie again) and `pkill`s leftover hung uiautomator processes between retries.
- **Verified:** offline selftest still 14/14; app sources untouched (harness-only change); real verdict = CI run #2 (this commit).
- **Run #2 addendum (same day):** run #2 proved the *notification-text* oracle wrong — `dumpsys notification_manager` records carry package/id/channel but **not the notification text**, so waiting for "Fetching subtitles" in that dump can never match. Final oracle design: `adb root` (available on AOSP emulator images) + direct read of the app-private `jobs.json` = the exact Queued→Done/Error state machine, zero UI dependence; notification assertions match record **ids** instead (1001 = ongoing foreground, 2000+jobId = per-job result); uiautomator dumps only on settled screens for the visual assertions. Run #3 = verdict.
- **Run #3 addendum:** the jobs.json oracle worked (job lifecycle verified end-to-end: `Queued → Error` with the bot-check message). Remaining gaps then diagnosed: (a) notification assertions made format-independent via **package presence**, plus `notif_dump.txt`/package dumps saved as artifacts and an "Allow" tap if a permission dialog appears; (b) with job rows on screen the window never reached accessibility idle because the 1s poller rebuilt all rows every tick → fixed in the app as v2.5.4 (§31).
- **Run #4 addendum:** v2.5.4 did its job — the terminal UI dump succeeded and the visual asserts passed (`list row shows ['ERROR']`, result line rendered). The two remaining notification FAILs were harness bugs, now fixed: (a) `dumpsys notification_manager` is not a ServiceManager name (the service is `notification`) — the oracle had been dumping an error string since run #1, so notifications were never actually observed (and may well have been fine all along); (b) the share-intent test used `--eu` (Uri extra) instead of `--es` (String extra), so `ShareActivity` correctly received null text and refused the share — the app was right, the test was wrong. Run #5 = verdict.
- **Run #5 addendum — CI verified the live success path:** 19/20 checks passed, including `share intent: list shows both rows ['DONE', 'ERROR']` — the shared link's subtitles were **fetched, formatted and saved for real from the GitHub runner** (ANDROID Innertube client worked from that Azure IP; the paste-flow link hit the bot check, showing both outcomes are handled). Foreground + result notification asserts passed (notifications were fine all along since run #1). Last FAIL was cosmetic harness: AOSP dialog themes expose button labels uppercased in the accessibility tree ("CLEAR"), so the exact-text `Clear` match missed; now matched case-insensitively on `android.widget.Button`. Run #6 = verdict.

## 31. v2.5.4 — job list redraws only when data changes (Sep 21, 2026)

- **Found by:** the CI emulator (run #3) — with one or more job rows visible, `uiautomator dump` timed out because the screen never reached accessibility idle; the 1s poller called `adapter.updateJobs()` (clear + addAll + notifyDataSetChanged, recreating every row view) on every tick even when nothing changed. Dumps only ever succeeded on the empty list.
- **Fix (`MainActivity.loadJobs`):** build a signature of the job set (`id:status:result;` per row); call `updateJobs` only when the signature differs from the last rendered one. `topProgress` visibility still refreshes every tick (cheap, no view rebuild). Empty↔non-empty and CLEAR transitions change the signature, so behavior is otherwise identical.
